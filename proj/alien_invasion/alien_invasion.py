...
class AlienInvasion:
    ...
    def _check_keyup_events(self, event):
        """Реагирует на ртпускание клавиш."""
        if event.key == pygame.K_RIGHT:
    ...
    def _update_bullets(self):
        ...
        self._check_bullets_allien_collisions()

    def _check_bullets_allien_collisions(self):
    ...
    def _update_aliens(self):
        """
        Проверьте, находится ли флот на краю,
    ...
    def _create_fleet(self):
        """Создание флота."""
        # Создайте пришельца и найдите количество пришельцев в ряду.
        # Расстояние между каждым пришельцем равно одной ширине пришельца.
        alien = Alien(self)
        alien_width, alien_height = alien.rect.size
        available_space_x = self.settings.screen_width - (2 * alien_width)
        number_aliens_x = available_space_x // (2 * alien_width)

        # Определите количество рядов пришельцев, которые помещаются на экране.
        ship_height = self.ship.rect.height
        available_space_y = (self.settings.screen_height -
                             (3 * alien_height) - ship_height)
        number_rows = available_space_y // (2 * alien_height)

        # Создайте полный флот пришельцев.
